...
The folder is typically the `icp-screened-TEMP-YYYY-MM-DD/` produced by `icp-screener`, with a `phase-map-*.md` artifact already inside it (produced by `phase-map-analyst`).
...
- **Sibling skills:** [`opportunity-analyst-reset`](../opportunity-analyst-reset/) (strip prior output for a clean re-run), [`phase-map-analyst`](../phase-map-analyst/) (upstream — defines the phase vocabulary), [`opportunity-sizer`](../opportunity-sizer/) (downstream — clusters and scores extracted opportunities).
...
- **Importance is inferred, not invented.** A 1-5 score is added only when the customer's words clearly signal it (intensity language, repetition, stated stakes). Absence is left as "not stated" rather than guessing a middle value.
